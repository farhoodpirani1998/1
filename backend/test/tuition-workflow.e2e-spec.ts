...
describe('Tuition plan -> installment workflow (e2e)', () => {
  ...
  describe('once a plan exists', () => {
    ...
    it('splits an amount that does not divide evenly, with the sum still matching finalAmount exactly', async () => {
      // 99,000,000 / 4 = 24,750,000 exactly divisible on purpose above;
      // use a plan whose finalAmount does NOT divide evenly by the
      // installment count to exercise the remainder-on-last-installment
      // logic (see InstallmentsService.generate).
      ...
      const sum = genRes.body.reduce((acc: number, i: any) => acc + Number(i.amount), 0);
    ...
    it('sums installment amounts to exactly the plan finalAmount', async () => {
      ...
      const sum = res.body.reduce((acc: number, i: any) => acc + Number(i.amount), 0);

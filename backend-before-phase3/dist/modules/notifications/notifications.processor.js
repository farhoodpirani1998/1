...
let NotificationsProcessor = NotificationsProcessor_1 = class NotificationsProcessor extends bullmq_1.WorkerHost {
    ...
    buildText(notification, context) {
        switch (notification.template) {
            case notification_entity_1.NotificationTemplate.WELCOME:
                return `${notification.student.fullName} عزیز، ثبت‌نام شما در دفتر مدرسه با موفقیت انجام شد. خوش آمدید.`;
            case notification_entity_1.NotificationTemplate.PAYMENT_CONFIRMATION: {
            ...
            case notification_entity_1.NotificationTemplate.OVERDUE_REMINDER:
            default:
                return `دانش‌آموز ${notification.student.fullName}: قسط به مبلغ ${Number(notification.installment?.amount ?? 0).toLocaleString('fa-IR')} تومان سررسید شده است.`;
        }
    }
};
...
exports.NotificationsProcessor = NotificationsProcessor = NotificationsProcessor_1 = __decorate([
    ...
    __metadata("design:paramtypes", [typeof (_a = typeof typeorm_2.Repository !== "undefined" && typeorm_2.Repository) === "function" ? _a : Object, sms_provider_service_1.SmsProviderService,
